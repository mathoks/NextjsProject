...
const page = async ({params:{id, storeOwner}}) => {
  ...
  const response =  await fetch(
    ...
    if(branchInfo?.data?.branches?.length === 0){
      return (
      <div className='shadow_cus p-6' >
      <p className='mx-auto'>no branches availiable</p>
          <span className='flex justify-end'>
          <button className={`ring-1 ring-gray-400 rounded-full px-2.5 py-1.5 ${id === session.user?.id ? 'visible' : "invisible"}`}><Link href={`/Dashboard/${encodeURIComponent(session.user.name)}/settings/branch`}>Add a branch</Link></button>
          </span>
        </div>)
