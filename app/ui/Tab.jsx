...
const Tabs = [{"id": 0, "val": "all Stores",  "href": "#", 'icon': <AllOutOutlined fontSize='small'/> }, {"id": 1, "val": "phone",  "href": "#", 'icon': <PhoneAndroid fontSize='small'/> },{"id": 2, "val": "furniture",  "href": "#", 'icon': <ChairAltOutlined fontSize='small'/> },{"id": 3, "val": "fashion",  "href": "#", 'icon': <ShoppingBagOutlined fontSize='small'/> },{"id": 4, "val": "machinery",  "href": "#" , 'icon': <BuildOutlined fontSize='small'/>},{"id": 5, "val": "building-Materials",  "href": "#", 'icon': <RoofingOutlined fontSize='small'/>}, {"id": 6, "val": "electronics",  "href": "#", 'icon': <TvOutlined fontSize='small'/> }]
  ...
  return (
    <section className='flex items-start justify-between  overflow-x-scroll gap-8 bg-white  w-screen ' >
    <ul role='list'  onClick={handleChange} className='pl-2 pr-2 flex items-start justify-between  overflow-x-scroll gap-8 text-gray-700  pt-2 w-screen'>
      { Tabs.map((tab, id)=>(
        ...
        <li key={tab.id} value={tab.id || id}  className={`first-letter:capitalize cursor-pointer focus:font-medium pb-[5px] text-nowrap  ${index === tab.id? 'border-b-2 transition-transform duration-300 border-indigo-700 text-base  icon_slate' : 'border-none text-sm' }`}>
