...
const Tabs = [{"id": 0, "val": "all Stores",  "href": "#", 'icon': <SelectAllOutlined fontSize='small'/> }, {"id": 1, "val": "phone",  "href": "#", 'icon': <PhoneAndroid fontSize='small'/> },{"id": 2, "val": "furniture",  "href": "#", 'icon': <ChairAltOutlined fontSize='small'/> },{"id": 3, "val": "fashion",  "href": "#", 'icon': <ShoppingBagOutlined fontSize='small'/> },{"id": 4, "val": "machinery",  "href": "#" , 'icon': <BuildOutlined fontSize='small'/>},{"id": 5, "val": "building-Materials",  "href": "#", 'icon': <RoofingOutlined fontSize='small'/>}, {"id": 6, "val": "electronics",  "href": "#", 'icon': <TvOutlined fontSize='small'/> }]
  ...
  return (
    <section className={`flex items-start justify-between  overflow-x-scroll gap-8   pb-0 z-50 w-full`} >
    <ul role='list'  onClick={handleChange} className='pl-4 pr-4 flex items-start justify-between  overflow-x-scroll gap-8 text-slate-400  pt-2'>
      { Tabs.map((tab, id)=>(
        
        <li key={tab.id} value={tab.id || id}  className={`border-opacity-0 first-letter:capitalize cursor-pointer focus:font-medium pb-[8px] text-nowrap  ${index === tab.id? ' border-opacity-100 transition-opacity  text-base  text-white' : 'transition-opacity duration-300 ease-linear border-opacity-0 text-sm' }`}>
