    ...
    <div className='flex flex-col space-y-2 bg-blue-50 rounded-md p-2 flex-grow'>
        ...
        <div className=''>
         ...
         <span className='flex'> <TagOutlined/> <p>tagged categories </p></span>
          <ul className='overflow-x-scroll flex space-x-2 py-2 w-[70vw] pl-2'>{categorys.map(({category: {name}}, ids)=> <li key={ids} className='ring-1 rounded-full px-2.5 py-0.5 shadow bg-white text-nowrap  text-blue-500 text-center text-sm'>{name}</li>)}</ul>
          </div> : ''}
        ...
        <div className='space-y-2'>
          ...
          <div className='pl-2'> 
         <span className='flex'> <LinkOutlined className='text-blue-400'/> <p>tagged products </p></span>
          <ul className='overflow-x-scroll flex space-x-2 py-2 w-[70vw] '>{products?.map(({product: {name, storeId}, productId}, ids)=> <Link key={ids} className='ring-1 rounded-full px-2.5 py-0.5 shadow bg-white  text-blue-500 text-center text-sm' href={`/store/${storeId}/product/${productId}`}>{name}</Link>)}</ul>
          </div> : ''}
        </div>
