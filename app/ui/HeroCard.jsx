...
const HeroCard = () => {
  ...
  return (
    <div className="bg-gradient-to-t relative from-white via-slate-400 to-slate-600 flex flex-col w-full">
      <div>
        <Carousel slides={slides} autoSlide={true} />
      </div>
      <div className="flex gap-2 overflow-x-scroll absolute inset-y-[12rem] h-40 z-40 w-full pl-2 pr-2">
        {[Logo, Logo2, Logo3, Logo1, Logo2, Logo3, Logo].map((img, id) => (
          <div key={id} className=" bg-white flex flex-col space-y-0 rounded-t-md  overflow-y-clip  shadow-md">
            <div className="text-left p-4 text-nowrap rounded-r-md rounded-l-md">
              <h3>Amazing Gifts</h3>
            </div>
            <Image
              src={img}
