...
const Header = async ({ params, message, storeInfo }) => {
    ...
    >
      ...
      <div>
        <div className="flex space-x-1 items-center pt-1">
          ...
          <span>
            <p>{message ? "" : market + " " + state + " " + country}</p>
          </span>
        </div>
      </div>
      <div className="flex justify-between items-center">
        ...
        <button
          className={` font-semibold ring-1 ring-[#6A0DAD] bg-white text-gray-800 rounded-full px-2.5 py-1.5 ${
            id === session?.user?.id ? "visible" : "invisible"
          }`}
        >
          Edit store
        </button>
